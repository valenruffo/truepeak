    ...
    try {
      ...
      if (authError) {
        if (authError.message.toLowerCase().includes("already registered")) {
          // Fallback to login if they already exist in Supabase auth but are missing a Postgres profile
          const { data: loginData, error: loginError } = await supabase.auth.signInWithPassword({
            email,
            password,
          });
          if (loginError) throw new Error(authError.message); // throw original error if login fails
