  ...
  return (
    <div className="rounded border p-8" style={{ borderColor: "#27272a", background: "#111114" }}>
      ...
      <form onSubmit={handleSubmit} className="space-y-4">
        <div>
          <label className="text-sm font-medium mb-1.5 block">Email o nombre del sello</label>
          <input type="text" value={identifier} onChange={(e) => setIdentifier(e.target.value)} className="w-full px-3 py-2.5 rounded border text-sm bg-transparent" style={{ borderColor: "#27272a" }} placeholder="tu@email.com o tu sello" required />
        </div>
        <div>
          <label className="text-sm font-medium mb-1.5 block">Contraseña</label>
          <div className="relative">
            <input type={showPassword ? "text" : "password"} value={password} onChange={(e) => setPassword(e.target.value)} className="w-full px-3 py-2.5 pr-10 rounded border text-sm bg-transparent" style={{ borderColor: "#27272a" }} placeholder="••••••••" required minLength={8} />
